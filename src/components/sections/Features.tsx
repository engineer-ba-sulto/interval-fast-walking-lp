...
const features = [
  {
    icon: <Timer size={32} className="text-[#558B2F]" />,
    title: "あなたに最適なタイマーを自動提案",
    description:
      "あなたの体力レベルに合わせて、効果的なトレーニング時間を自動で設定します。",
  },
  {
    icon: <Footprints size={32} className="text-[#558B2F]" />,
    title: "2種類のトレーニングモード",
    description:
      "目的やその日の体調に応じて、2つの異なるウォーキングモードから選択できます。",
  },
  {
    icon: <BarChart3 size={32} className="text-[#558B2F]" />,
    title: "視覚的で分かりやすい進捗表示",
    description:
      "日々の成果がグラフで一目でわかり、モチベーション維持に繋がります。",
  },
  {
    icon: <MousePointerClick size={32} className="text-[#558B2F]" />,
    title: "シンプルで簡単な操作",
    description:
      ...
      <div className="container mx-auto px-6 relative z-10">
        <FadeIn>
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-[#212121] mb-6">
              あなたの運動習慣を成功に導く4つの力
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {features.map((feature, index) => (
            <FadeIn key={index} delay={index * 150} className="h-full">
              <div
                className="bg-white p-8 rounded-[2rem] h-full flex flex-col items-start text-left transition-transform hover:-translate-y-1 duration-300"
                style={{
                  boxShadow:
              ...
              >
                ...
                <h3 className="text-lg font-bold text-[#212121] mb-4 leading-tight">
                  {feature.title}
                </h3>
